-- Draft bearer values are derived only at the feedback boundary.  The action
-- result and the draft-grant table retain hashes; the plaintext is returned
-- once to the workflow caller and can be reconstructed for an exact replay.
update public.heartbeat_action_tokens
   set result = result - 'draft_grant'
 where result ? 'draft_grant';
...
declare token_row record; item_row record; allowed boolean; to_status text;
    ...
    event_key text; result_value jsonb; return_value jsonb;
    draft_token text; draft_hash text; draft_version integer;
begin
    select t.*, d.run_id, d.token_seed, r.profile_id into token_row
    ...
    if token_row.consumed_at is not null then
        if token_row.provider_event_key=p_provider_event_key and token_row.consumed_by_principal=p_actor_ref and token_row.result is not null then
            if token_row.action = 'prepare_draft' then
                draft_version := (token_row.result->>'version')::integer;
                draft_token := encode(sha256(convert_to(token_row.token_seed || ':prepare_draft:' || token_row.item_id::text || ':' || draft_version::text || ':' || p_actor_ref || ':' || p_provider_event_key, 'UTF8')),'hex');
                return token_row.result || jsonb_build_object('draft_grant', draft_token);
            end if;
            return token_row.result;
        end if;
    ...
    if token_row.action = 'prepare_draft' then
      draft_token:=encode(sha256(convert_to(token_row.token_seed || ':prepare_draft:' || item_row.item_id::text || ':' || new_version::text || ':' || p_actor_ref || ':' || p_provider_event_key, 'UTF8')),'hex');
      draft_hash:=encode(sha256(convert_to(draft_token,'UTF8')),'hex');
    ...
    if draft_token is not null then
        return_value := result_value || jsonb_build_object('draft_grant', draft_token);
        return return_value;
    end if;
    return result_value;
end; $$;
...
grant execute on function heartbeat_consume_action(text,text,text) to centaur_heartbeat_feedback;
