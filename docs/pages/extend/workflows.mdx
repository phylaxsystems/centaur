...
When a managed workflow needs Postgres, grant its principal a typed `pg_dsn`
secret through iron-control. An agent-k8s workflow-host sandbox receives the
selected database as `WORKFLOW_HOST_DATABASE_NAME`; iron-proxy separately
injects the principal-scoped `CENTAUR_POSTGRES_DSN`. The control plane's raw
`DATABASE_URL` is never forwarded to that sandbox. Local workflow hosts retain
the direct `DATABASE_URL` development path. The Python host prefers the proxy
DSN, adds the explicit database, and uses an asyncpg pool reset that preserves
the proxy-enforced role and settings.
...
## Start a workflow from an internal event

Use `EVENT_TRIGGERS` when a durable workflow should start from an event that
already entered Centaur, such as a signed Slack block action:

```python
EVENT_TRIGGERS = [
    {
        "name": "report-review-actions",
        "event_name_prefix": "slack.block_action.acme.report.",
    }
]
```

Each trigger name must be unique. A matching `POST /api/workflows/events`
request must include `idempotency_key`; Centaur combines that key with the
trigger name so ingress retries return the existing workflow run instead of
starting another. The handler receives:

```json
{"event_name":"slack.block_action.acme.report.approve","payload":{}}
```

Event triggers start new runs. They do not replace `ctx.wait_for_event(...)`,
which resumes an already-running workflow. One emitted event can still wake
waiters and start declared trigger workflows.

## Durable heartbeat state

Trusted, managed-principal workflows can use `ctx.heartbeat` for recurring
review loops. The typed API owns profile registration, source checkpoints,
immutable observations, material-change items, version-checked synthesis,
content-hashed replay artifacts, delivery/action tokens, review transitions,
and evidence-linked semantic-memory proposals. Organization overlays own the profile policy and source
normalization; they should not issue SQL directly.

Heartbeat profiles are organization, team, or personal scoped. Every run and
profile operation is checked against the workflow's managed principal. Review
actions additionally require an expiring single-use token, the rendered item
version, and an explicit profile reviewer grant. Memory facts begin as
`proposed`; an overlay must define a separate confirmation/retrieval policy
before treating them as agent context.

The built-in heartbeat migrations create separate no-login database roles for
the collection/run path and the narrower human-feedback path. Deployments
should grant the matching typed DSN to each managed workflow principal rather
than sharing the control-plane database credential.
...
Direct administrative API calls need a short-lived Console service token. Mint
one from the running Console instead of storing a separate static key:
...
List recent runs, optionally filtering by exact workflow name. The default
limit is 50 and the maximum is 1000:
