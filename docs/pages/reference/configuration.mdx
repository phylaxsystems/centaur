...
| `apiRs.executionMaxDurationMs` / `SESSION_EXECUTION_MAX_DURATION_MS` | Helm value, default `0` (unbounded). | Wall-clock ceiling applied to an execution whose caller sets no `max_duration_ms`. Without it a turn that keeps streaming model output never trips an idle path and runs until an operator intervenes; with it the execution fails with the distinct `execution exceeded max_duration_ms=...` error and the surface reports it. A per-request `max_duration_ms` always wins. |
...
| `apiRs.sessionEventsRetentionDays` / `SESSION_EVENTS_RETENTION_DAYS` | Helm value, default `0` (disabled). | Deletes `session_events` older than this many days on each cleanup sweep. The table carries one row per harness stdout line and nothing removed them, so it grows for the life of the deployment — order 450MB/day on a modest single node against a 20Gi default volume. Events belonging to a `queued` or `running` execution are never deleted, whatever their age. Sweeps are bounded per pass, so a large first backlog drains over several. |
| `apiRs.sandboxAdmissionWaitSecs` / `SESSION_SANDBOX_ADMISSION_WAIT_SECS` | Helm value, default `600`. | How long an admission blocked by `sandboxRunningLimit` waits for a running slot before failing the turn. Admission is a limit rather than a queue, so a refusal is terminal — nothing retries it and nothing surfaces on the originating thread — while the slot it needed usually frees seconds later. The wait records one `session.sandbox_capacity_waiting` event so the state is visible. `0` fails immediately. |
...
| `SESSION_SANDBOX_MEMORY_BUDGET_GIB` | `apiRs.sandboxMemoryBudgetGib`. | Memory budget in GiB committed by the running session sandboxes. Admission enforces the smaller of `sandboxRunningLimit` and the budget divided by one sandbox's committed share (`sandbox.resources.limits.memory` plus the iron-proxy memory limit), and records the computation in the `sandbox_capacity_admitted` and `sandbox_capacity_waiting` events and the capacity refusal error. Requires `sandboxRunningLimit` > 0 and a parseable agent memory limit; a budget that cannot hold one sandbox fails startup. Default `0` (disabled). |
