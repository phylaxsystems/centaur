...
`SLACK_ETL_EXCLUDED_CHANNEL_PATTERNS` applies to this user-scoped private-channel
path as well as the workflow-owned Slack ETL. Matching private channels are
filtered before membership or history requests are made.
...
| `SLACK_ETL_EXCLUDED_CHANNEL_PATTERNS` | empty | Comma-separated channel-name globs or exact channel IDs to skip, without needing the leading `#`. Prefer an exact ID when a rename must not bypass the exclusion. |
...
Each incremental sync reconciles these patterns against both the current Slack
channel list and previously stored public/private channel records. Matching
channel rows, queued backfills, messages, attachments, and derived documents
are deleted. RLS also hides every channel marked non-syncable, so a stale row
cannot remain retrievable while cleanup is pending.
